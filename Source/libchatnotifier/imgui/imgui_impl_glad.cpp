...
struct ImGui_ImplGlad_Data {
	...
	ImGui_ImplGlad_Data() { memset((void *)this, 0, sizeof(*this)); }
};
...
static ImGui_ImplGlad_Data *ImGui_ImplGlad_GetBackendData() {
	return ImGui::GetCurrentContext()
			   ? static_cast<ImGui_ImplGlad_Data *>(ImGui::GetIO().BackendRendererUserData)
			   : nullptr;
}

// OpenGL vertex attribute state (for ES 1.0 and ES 2.0 only)
#ifndef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
struct ImGui_ImplGlad_VtxAttribState {
...
// Functions
bool ImGui_ImplGlad_Init(const char *glsl_version) {
	ImGuiIO &io = ImGui::GetIO();
	IMGUI_CHECKVERSION();
	IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a renderer backend!");

	// Setup backend capabilities flags
	auto bd = IM_NEW(ImGui_ImplGlad_Data)();
	io.BackendRendererUserData = static_cast<void *>(bd);
	io.BackendRendererName = "imgui_impl_glad";
...
#else
	std::println("Getting GL version");
	// Desktop or GLES 3
	auto gl_version_str = reinterpret_cast<const char *>(glGetString(GL_VERSION));
	GLint major = 0;
	GLint minor = 0;
	...
	*/
	...
	for (GLint i = 0; i < num_extensions; i++) {
		if (const auto extension = reinterpret_cast<const char *>(glGetStringi(GL_EXTENSIONS, i));
			extension != nullptr && strcmp(extension, "GL_ARB_clip_control") == 0)
...
void ImGui_ImplGlad_Shutdown() {
	ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
	IM_ASSERT(bd != nullptr && "No renderer backend to shutdown, or already shutdown?");
	ImGuiIO &io = ImGui::GetIO();

	ImGui_ImplGlad_DestroyDeviceObjects();
...
void ImGui_ImplGlad_NewFrame() {
	const ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
	IM_ASSERT(bd != nullptr &&
			  "Context or backend not initialized! Did you call ImGui_ImplGlad_Init()?");

	if (!bd->ShaderHandle) ImGui_ImplGlad_CreateDeviceObjects();
	if (!bd->FontTexture) ImGui_ImplGlad_CreateFontsTexture();
}

static void ImGui_ImplGlad_SetupRenderState(const ImDrawData *draw_data, const int fb_width,
											const int fb_height, const GLuint vertex_array_object) {
	const ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
...
void ImGui_ImplGlad_RenderDrawData(ImDrawData *draw_data) {
	...
	ImGui_ImplGlad_VtxAttribState last_vtx_attrib_state_pos;
	last_vtx_attrib_state_pos.GetState(bd->AttribLocationVtxPos);
	ImGui_ImplGlad_VtxAttribState last_vtx_attrib_state_uv;
	last_vtx_attrib_state_uv.GetState(bd->AttribLocationVtxUV);
	ImGui_ImplGlad_VtxAttribState last_vtx_attrib_state_color;
...
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
	const GLboolean last_enable_primitive_restart =
...
#endif
	ImGui_ImplGlad_SetupRenderState(draw_data, fb_width, fb_height, vertex_array_object);
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		// issues.
		const GLsizeiptr vtx_buffer_size = static_cast<GLsizeiptr>(cmd_list->VtxBuffer.Size) *
										   static_cast<int>(sizeof(ImDrawVert));
		const GLsizeiptr idx_buffer_size =
			static_cast<GLsizeiptr>(cmd_list->IdxBuffer.Size) * static_cast<int>(sizeof(ImDrawIdx));
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != nullptr) {
				...
				if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
					ImGui_ImplGlad_SetupRenderState(draw_data, fb_width, fb_height,
													vertex_array_object);
				else
				...
				GL_CALL(glScissor(static_cast<int>(clip_min.x),
								  static_cast<int>((float)fb_height - clip_max.y),
								  static_cast<int>(clip_max.x - clip_min.x),
								  static_cast<int>(clip_max.y - clip_min.y)));

				// Bind texture, Draw
				GL_CALL(
					glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>((intptr_t)pcmd->GetTexID())));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
				if (bd->GlVersion >= 320)
...
bool ImGui_ImplGlad_CreateFontsTexture() {
...
void ImGui_ImplGlad_DestroyFontsTexture() {
	const ImGuiIO &io = ImGui::GetIO();
	ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
	if (bd->FontTexture) {
...
static bool CheckShader(const GLuint handle, const char *desc) {
	...
	if (static_cast<GLboolean>(status) == GL_FALSE)
		fprintf(stderr,
				"ERROR: ImGui_ImplGlad_CreateDeviceObjects: failed to compile %s! With GLSL: %s\n",
...
static bool CheckProgram(const GLuint handle, const char *desc) {
	...
	if (static_cast<GLboolean>(status) == GL_FALSE)
		fprintf(stderr,
				"ERROR: ImGui_ImplGlad_CreateDeviceObjects: failed to link %s! With GLSL %s\n",
				desc, bd->GlslVersionString);
	if (log_length > 1) {
		ImVector<char> buf;
		buf.resize((int)(log_length + 1));
...
bool ImGui_ImplGlad_CreateDeviceObjects() {
	ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
	...
	const auto vertex_shader_glsl_120 = "uniform mat4 ProjMtx;\n"
	...
	const auto vertex_shader_glsl_130 = "uniform mat4 ProjMtx;\n"
										"in vec2 Position;\n"
										"in vec2 UV;\n"
										"in vec4 Color;\n"
										"out vec2 Frag_UV;\n"
	...
	const auto vertex_shader_glsl_300_es = "precision highp float;\n"
	...
	const auto vertex_shader_glsl_410_core = "layout (location = 0) in vec2 Position;\n"
											 "layout (location = 1) in vec2 UV;\n"
											 "layout (location = 2) in vec4 Color;\n"
											 "uniform mat4 ProjMtx;\n"
											 "out vec2 Frag_UV;\n"
	...
	bd->AttribLocationVtxPos =
		static_cast<GLuint>(glGetAttribLocation(bd->ShaderHandle, "Position"));
	bd->AttribLocationVtxUV = static_cast<GLuint>(glGetAttribLocation(bd->ShaderHandle, "UV"));
	bd->AttribLocationVtxColor =
		static_cast<GLuint>(glGetAttribLocation(bd->ShaderHandle, "Color"));

	// Create buffers
	glGenBuffers(1, &bd->VboHandle);
	glGenBuffers(1, &bd->ElementsHandle);

	ImGui_ImplGlad_CreateFontsTexture();
...
void ImGui_ImplGlad_DestroyDeviceObjects() {
	ImGui_ImplGlad_Data *bd = ImGui_ImplGlad_GetBackendData();
	if (bd->VboHandle) {
	...
	ImGui_ImplGlad_DestroyFontsTexture();
}
