...
public:
	static auto initialize() -> Result {
		...
		python_runner.add_job_sync([]() -> void {
			...
			std::println("Init isolated pre-config..");
			PyPreConfig_InitIsolatedConfig(&preconfig);
			std::println("Pre-initialize..");
			...
			std::println("Init isolated config..");
			...
			std::println("Initialize interpreter..");
			py::initialize_interpreter();

			std::println("Add scripts paths..");
	...
	static auto get_scripts() -> std::vector<Script *> {
		...
		for (const auto &script : scripts | std::views::values)
			script_ptrs.emplace_back(script.get());
